      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			//BEHAVIOUR PLANNER BELOW
			bool too_close = false;
		
			// for each other car detected
			for(int i = 0; i < sensor_fusion.size(); i++){
				
				float d = sensor_fusion[i][6];
				
				//is car in front of ego
				if(d < (2+4*lane+2) && d > (2+4*lane-2)){ //get velocity and s
						...
						if(lane == 1){
							...
							for(int i = 0; i < sensor_fusion.size(); i++){
								...
								if(d < (2+4*(lane-1)+2) && d > (2+4*(lane-1)-2)){
									...
									if (((front_c-check_car_s) < 15 && (front_c > check_car_s) ) 
			...
			// create list of trajectory points
			vector <double> ptsx;
			vector <double> ptsy;

			// reference state, can be either car's position or path's end point
			double ref_x = car_x;
			double ref_y = car_y;
			...
			// use the previous path's end point a starting reference
			else
			{
